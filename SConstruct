...
if GetOption("clean"):
   ...
   SConscript('source/gtRecs/SConscript', variant_dir='build/gtRecs/', duplicate=0)
   SConscript('source/mongoUpdate/SConscript', variant_dir='build/mongoUpdate/', duplicate=0)
   SConscript('source/gtMongoUpdate/SConscript', variant_dir='build/gtMongoUpdate/', duplicate=0)
   SConscript('source/mongoDataPrep/SConscript', variant_dir='build/mongoDataPrep/', duplicate=0)
   SConscript('source/gtMongoVideoRollDataPrep/SConscript', variant_dir='build/gtMongoVideoRollDataPrep/', duplicate=0)
   SConscript('source/mongoConversationFixer/SConscript', variant_dir='build/mongoConversationFixer/', duplicate=0)
...
# mongoCommentsGetter tries to identify the language, which requires a library that's a PITA to build with scons
#
